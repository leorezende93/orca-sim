...
private:	
	...
	int16_t _a_buffer[N][N]; 
	int16_t _b_buffer[N][N];
		
	int32_t _z_buffer[N][N];
	...
	// Flags
	int _init_array_flag;
	int _start_mult_flag;
	int _shift_out_flag; 
	int _end_eop_flag;

public:
	// Setters
	void SetABuffer(int i, int j, int16_t data);
	void SetBBuffer(int i, int j, int16_t data);
	void SetInit(int init);
    
    // Getters
    int32_t GetAdderValue();
    int GetEOP();
