...
void TSystolicArray::SetABuffer(int i, int j, uint64_t data){
	_a_buffer[i][j] = data;
}
	
void TSystolicArray::SetBBuffer(int i, int j, uint64_t data){
	_b_buffer[i][j] = data;
}
...
int64_t TSystolicArray::GetAdderValue(){
	return _acc;
}
	...
	switch(_systolic_array_state){			
			...
			if (DEBUG == 1) {
				if (_eop == 0){
					...
					for (x = 0; x < N; x++){
						for (y = 0; y < N; y++){
							printf("%lu ", _PE[x][y]->GetMACResult());
						}
						printf("\n");
					}
