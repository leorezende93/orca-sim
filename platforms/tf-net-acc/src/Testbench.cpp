...
void Testbench::printLayer(int n_filter, int h, int w, int64_t map[]) {
	int c,idx1,idx2;
	
	for (c = 0; c < n_filter; c++){
		for (idx1 = 0; idx1 < h; idx1++){
			for (idx2 = 0; idx2 < w; idx2++){
				printf("%ld ",map[(h*w*c)+ (h*idx1) + idx2]);
			}
			printf("\n");
		}
	printf("\n");
...
void Testbench::StoreOfmap(int64_t layer_ifmap[], int32_t bias[], int *layer_flag, int input_c, int output_c){
	_wait_eop = 0;
	_array->SetInit(0);
...
int Testbench::Dense(int32_t weight[], int64_t ifmap[], int32_t bias[], int64_t ofmap[],int n_channel, int input_h, int input_w, int input_c){
	int i,j;
	int filter_len;
	int aux,predict = 0;
